...
class ExceptioExceptionsTable < ActionTable::ActionTable
  ...
  column(:message, sortable: false) { |exception| exception.message.to_s.truncate(40) }
  column(:code_location) { |exception| exception.code_location.to_s.gsub(Rails.root.to_s, '').truncate(20) }
  column(:count, sort_field: :instances_count ) { |exception| exception.instances_count }
  column(:last_occurence, sort_field: :updated_at) { |exception| ln exception.updated_at }
  column(:actions, sortable: false) { |exception| link_to '<i class="fal fa-trash"></i>'.html_safe, exceptio.exception_path(exception), data: { turbo_method: :delete }}
